...
## Overview
Notes & Gallery is a Laravel-based web application that allows users to register, log in, write and manage personal notes, and upload/manage photos securely in their own gallery.  
The app features authentication, file storage, custom UI for login/register pages, and user profile management.
...
### 3️ Install Node dependencies (optional)
```bash
npm install
npm run build
```
...
### 5️ Import SQL Database
(Instruction above)

### 6️ Run migrations (if needed)
```bash
php artisan migrate
```
